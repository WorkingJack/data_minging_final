...
def load_movielens(train_dir, batch, use_recs=False):
  ...
  train = np.loadtxt("%s/book_train.tsv"%train_dir, delimiter='\t')
  valid = np.loadtxt("%s/book_train.tsv"%train_dir, delimiter='\t')
  if use_recs:
    ...
    test = np.loadtxt("%s/book_train.tsv" % train_dir, delimiter='\t')
